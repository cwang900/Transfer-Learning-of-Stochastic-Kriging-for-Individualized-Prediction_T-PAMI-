...
{ 
  ...
  #error-prone one the side                                     1,2,11,12
...
for(jj in 1:rep_time){
  ...
  for (hot_key in 1:3){
    ...
    x0<-c(rep(2,4*n-2),1,0.9,2,2,2,2)
     if (hot_key == 1){
  ...
  yvar_prop[[jj]]<-yvar1
  #record the RMSE
  plot(x = xstar,y = ystar, type = "l",lty = 1,col = "darkgreen",ylim = c(-5,10),main = paste("Proposed",jj))	
  ...
  RMSE[jj,1]<-sqrt(sum((ystar-ypred_correct[,1])^2)/length(ystar))
}
