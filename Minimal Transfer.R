...
{ 
  ...
  for(i in 1:(n-1)){
    ...
    cyi <- cyii(trainx[[i]],trainx[[i]],L = H[c(2*i-1,2*i,4*n-1,4*n)])
    zii[[i]] <- cyi$full
  ...
  #fill the same point on the side 811 to 945, no error, margin from 
  #error-prone one the side 
  ...
  return(list(table = sparseMatrix(i=pfi,j=pfj,x=c(b1,b2,zpp,b3,zss),symmetric=T)+ diag(0.00001,nrow = ppt,ncol = ppt),error = err_mat))
}
...
#find_long is used to get the single replication with the most data samples, tied pick at random. 
find_long<-function(list,si){
  rowcount<-sapply(1:num[si],function(j){sum(list[[j]] == 0)})
  picked<-which(rowcount == min(rowcount))
  if(length(picked) == 1){return(list[[picked]])}
  if(length(picked)>1){return(list[[sample(picked,1)]])}
}

trainy<-lapply(1:(n-1),function(i){find_long(trainy1[[i]],si = i)})
testy<-find_long(trainy31,n) 
trainx<-lapply(1:(n-1),function(i){tpoints[(trainy[[i]]!= 0)]})
testx<-testx_full[testy!=0]
#filter out zero entries 
trainy<-lapply(1:(n-1),function(i){trainy[[i]][trainy[[i]]!= 0]})
testy<-testy[testy!=0]


##--------------------------------------------------------------------------------
#record length and scale parameters 
H_Min<-lapply(1:4,function(i){array(NA,dim = c(rep_time,4*n+4))})
ypred_Min<-array(NA,dim = c(rep_time,length(tpoints)))
yvar_Min<-vector(mode = "list",length = rep_time)
...
pf=index(n,lengths(trainx),length(testx),length(xsame))	
pfi=pf$pfi;pfj=pf$pfj

gamma = 0.1
eta = 0.00001

y=c(unlist(trainy),testy,ysame) 
leny=length(y)

x0<-c(rep(2,4*n-2),1,0.9,2,2,2,2) #initial guess
...
#prediction for penalized version
zip_pred=list()
zip_pred =lapply(1:(n-1), function(i){cyip(trainx[[i]],xstar,H1[c(2*i-1,2*i,2*n+2*i-1,2*n+2*i)])})

D1=outer(xstar,testx,`-`) 
K1=H1[(2*n-1):(4*n)]
zip_pred[[n]]=t(Reduce("+",lapply(2:n, function(i){K1[(2*i-1)]^2*exp(-0.25*D1^2/K1[(2*i)]^2)+cyip(xstar,testx,H1[c(2*n-1,2*n,4*n+3,4*n+4)])})))

D11=outer(xstar,xsame,`-`) 
...
H_Min[[4]][jj,]<-H_Min[[hot_select]][jj,]
ypred_Min[jj,]<-ypred_correct[,1]
yvar_Min[[jj]]<-yvar1


plot(x = xstar,y = ystar, type = "l",lty = 1,col = "darkgreen",ylim = c(-5,10),main = paste("Min Trans ",jj))	
...
RMSE[jj,3]<-sqrt(sum((ystar-ypred_correct[,1])^2)/length(ystar))
}
